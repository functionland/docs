...
**Close MetaMask completely from your recent apps, then open it again** — from its icon, or by returning to the FxBlox tab and tapping **Open wallet to approve**. A freshly started MetaMask shows the waiting request straight away. Opening it again *without* closing it first only returns you to the same stuck screen. Your signature request stays valid throughout, so nothing is lost and you do not need to start setup again.
...
- **MetaMask sticks on its splash screen every time.** It is failing to resume from the background — a MetaMask fault, not something the page controls. Closing it from recent apps and opening it again always clears it. Check MetaMask is up to date and look at its **Auto-lock** setting; see the tip under [Set your identity](#3-set-your-identity).
